...
def text2emb(text, stop_words, model, pon):
    if text == None: 
        return []
    lang = langid.classify(text)[0]
    word_tokens = word_tokenize(text)
    filtered_sentence = [w.lower() for w in word_tokens if not w.lower() in stop_words[lang]]
    embeddings = [model[lang][token]*pon for token in filtered_sentence if token in model[lang].key_to_index]
    return embeddings
...
def clean_posts(posts, stop_words, model):
    ...
    for i in posts:
        ...
        for label in ["id", "title"]: # "description", "description2"
            emb += text2emb(i[label], stop_words, model, 1)

        if len(i["tags"]) > 5:
            i["tags"] = i["tags"][:5]

        for t in i["tags"]:
            emb += text2emb(t, stop_words, model, 1)
...
def clean_products(products, stop_words, model):
    ...
    for i in products:
        emb = []
        for label in ["descripcion", "Material de las patas", "Materiales", "Construcción de la estructura", "Estancias", "Estilo principal"]: #"Material principal", "Color principal",
            emb += text2emb(i[label], stop_words, model, 1)
        for label in ["Colores"]:
            emb += text2emb(i[label], stop_words, model, 1)
        for label in ["id"]:
            emb += text2emb(i[label], stop_words, model, 7)
        for label in ["Forma del producto"]:
            emb += text2emb(i[label], stop_words, model, 0.5)
